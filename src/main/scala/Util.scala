...
object Util
{
  def AlgStream(name: String) = {
    name match  {
      case _ => {//TODO implement pattern matching by name, fail if not found
        val gbpso = gbest(0.729844, 1.496180, 1.496180, Guide.pbest[Mem[Double], Double],
  ...
  def makeSwarm(bounds: NonEmptyVector[Interval], swarmSize: Int) = {
    ...
    Position.createCollection(PSO.createParticle(x => Entity(Mem(x, x.zeroed), x)))(bounds, size)
  }

  def makeProblem(name: String) = {
    val p = name match {
      case "f3" => Eval.unconstrained((x: NonEmptyVector[Double]) => {
        Feasible(benchmarks.cec.cec2005.Benchmarks.f3(mkAtLeast2List(x)))
      })
    case _ => Eval.unconstrained((x: NonEmptyVector[Double]) => {
      Feasible(benchmarks.cec.cec2005.Benchmarks.f3(mkAtLeast2List(x)))
    })
  }
...
def mkAtLeast2List(x: NonEmptyVector[Double]) =
  AtLeast2List.make(x) match {
    case ZValidation.Failure(_, e@_) => sys.error("Input vector requires at least 2 elements")
